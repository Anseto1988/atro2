...
| v2.8.0-alpha | — | — | F-MTFStretch (Midtone Transfer Function, Auto-BTF), F-BackgroundNeutralization (Auto/ROI-Modus, per-Kanal), F-LocalContrastEnhancement (CLAHE pure numpy, Luminanz/pro-Kanal), F-StarReduction (Minimum-Filter, Gaussian-Mask, Blend), F-ColorGrading (Shadow/Midtone/Highlight, 9 Shifts, luminanzbasiert) — +324 Tests, ~3997+ Gesamt |
| v2.9.0-alpha | — | — | F-ChannelBalance (Per-Kanal R/G/B Hintergrund-Offset, Auto-Sample), F-AIAligner (ONNX CNN, DoG-Keypoints, RANSAC, FR-2.2), F-StarAnalysis (PSF 2D-Gauss-Fit, FWHM, Elliptizität, Strehl), F-PipelineTimer (Step-Zeitmessung, Rolling-Average ETA) — +101 Tests, ~4098 Gesamt |
| v2.10.0-alpha | — | — | F-NAFNetDenoiser (NAFNet ONNX tile-basiert, OnnxDenoiseStep mit Fallback, Backend-Auswahl), F-FrameQualityDashboard (sortierbare Score-Tabelle, CSV-Export), F-ModelManager (Download/Verify/Delete ONNX-Modelle, QThread-Worker) — +115 Tests, ~4213 Gesamt |
